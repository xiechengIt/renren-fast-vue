<template>
  <el-dialog
    ...
    <el-form-item label="生产计划数量" prop="productPlanAmount">
      <el-input v-model="dataForm.productPlanAmount" placeholder="生产计划数量"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/productionplan/productplanmanage/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.specification = data.productPlanManage.specification
                this.dataForm.partNum = data.productPlanManage.partNum
                this.dataForm.productPlanAmount = data.productPlanManage.productPlanAmount
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/productionplan/productplanmanage/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
