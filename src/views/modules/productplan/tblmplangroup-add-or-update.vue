<template>
  <el-dialog
    ...
    <el-form-item label="组合订单号" prop="combineOrderNo">
      <el-input v-model="dataForm.combineOrderNo" placeholder="组合订单号"></el-input>
    </el-form-item>
    <el-form-item label="工单合计数量" prop="orderTotalCount">
      <el-input v-model="dataForm.orderTotalCount" placeholder="工单合计数量"></el-input>
    </el-form-item>
    <el-form-item label="分计划合计数量" prop="planTotalCount">
      <el-input v-model="dataForm.planTotalCount" placeholder="分计划合计数量"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          planTotalCount: [
            { required: true, message: '分计划合计数量不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.mplanId) {
            this.$http({
              url: this.$http.adornUrl(`/productionplan/tblmplangroup/info/${this.dataForm.mplanId}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.orderTotalCount = data.tblmplanGroup.orderTotalCount
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/productionplan/tblmplangroup/${!this.dataForm.mplanId ? 'save' : 'update'}`),
              method: 'post',
