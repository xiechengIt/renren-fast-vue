    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/stator/statorbaditemoption/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.optionName = data.statorBadItemOption.optionName
                this.dataForm.parentId = data.statorBadItemOption.parentId
                this.dataForm.optionLevel = data.statorBadItemOption.optionLevel
                this.dataForm.optionOrder = data.statorBadItemOption.optionOrder
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/stator/statorbaditemoption/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
