<template>
  <div class="">
    <div class="test">
      ...
      <el-button id="testJ" type="primary" round @click="testAll"
        >测试所有产线</el-button
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    testAll() {
      this.$http({
        url: "http://localhost:9000/board/department/allLine",
        method: "get",
      })
        .then(({ data }) => {
          let list = Array.from(data);
          list.forEach((item) => {
            window.open("http://localhost:9000/dechang/packageBoard/" + item);
          });
        })
        .catch((error) => {
          this.$message({
    ...
    //打开指定机号页面指定次数
    testByMachineId() {
      let url = "http://192.168.6.254:8000/dechang/board/" + this.testMachineId;
    ...
    open(num) {
      this.$confirm("确认重置?", "提示", {
        ...
        .then(() => {
          let url = "";
          if (num === 1) {
            url = "http://localhost:9000/board/department/loginResetAll";
          } else if (num === 2) {
            url = "http://localhost:9000/board/linehourlydata/reset";
          }
          this.$http({
            url: url,
            method: "get",
    ...
    reset(num) {
      ...
      if (num === 1) {
        url =
          "http://localhost:9000/board/department/loginReset/" +
          this.loginCountMachineId;
      } else if (num === 2) {
        url =
          "http://localhost:9000/board/linehourlydata/reset/" +
